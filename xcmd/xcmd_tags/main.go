...
import (
	...
	"flag"
	"fmt"
	"github.com/pubgo/g/pkg/structutil"
...
func (c *config) removeTags(tags *structutil.Tags) *structutil.Tags {
...
func (c *config) clearTags(tags *structutil.Tags) *structutil.Tags {
	if !c.clear {
		return tags
	}
...
func (c *config) clearOptions(tags *structutil.Tags) *structutil.Tags {
	if !c.clearOption {
		return tags
	}
...
func (c *config) removeTagOptions(tags *structutil.Tags) (*structutil.Tags, error) {
...
func (c *config) addTagOptions(tags *structutil.Tags) (*structutil.Tags, error) {
...
func (c *config) addTags(fieldName string, tags *structutil.Tags) (*structutil.Tags, error) {
	if c.add == nil || len(c.add) == 0 {
		return tags, nil
	}

	splitted := strutil.CamelCase(fieldName)
	name := ""
