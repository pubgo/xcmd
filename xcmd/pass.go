...
var (
	...
	getch = func(r io.Reader) (byte, error) {
		buf := make([]byte, 1)
...
// getPasswd
// returns the input read from terminal.
...
// GetPasswd
// returns the password read from the terminal without echoing input.
// The returned byte array does not include end-of-line characters.
func GetPasswd(prompt ...string) ([]byte, error) {
	if len(prompt) > 0 {
		defaultPrompt = prompt[0]
	}

	return getPasswd(defaultPrompt, false, os.Stdin, os.Stdout)
}

// GetPasswdMasked
// returns the password read from the terminal, echoing asterisks.
// The returned byte array does not include end-of-line characters.
func GetPasswdMasked(prompt ...string) ([]byte, error) {
	...
	return getPasswd(defaultPrompt, true, os.Stdin, os.Stdout)
}

// GetPasswdPrompt
// prompts the user and returns the password read from the terminal.
...
func restore(fd uintptr, oldState *terminalState) error {
	return xerror.Wrap(terminal.Restore(int(fd), oldState.state), "Restore Error")
}
